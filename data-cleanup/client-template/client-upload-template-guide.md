...
2. **Add the hidden mapping tab.** Add a sheet named `mappings`. Paste this:
   ...
   (Genre labels in `mappings!A2:A14`, handles in `B2:B14`. Formats need no mapping — the format label itself goes straight into the sheet.)
3. **Add dropdowns (Data → Data validation) on the helper columns:**
   - `Format` (col Q): list of items `VHS`, `DVD`, `Blu-Ray`, `4K`
   - `Genre 1/2/3` (cols R, S, T): list from range `mappings!A2:A14`
   - `Type` (col U): list of items `Rental`, `Floor Sale`
4. **Paste the formulas into row 2** of these columns, then select row 2 and drag the fill handle down as far as you need (formulas copy per row):
   ...
   | F `Tags` | `=TEXTJOIN(", ", TRUE, U2, Q2, R2, S2, T2, V2)` |
   | G `Status` | `="Active"` |
   ...
   | P `Genre (…shopify.genre)` | `=TEXTJOIN("; ", TRUE, IFERROR(VLOOKUP(R2,mappings!$A:$B,2,FALSE),""), IFERROR(VLOOKUP(S2,mappings!$A:$B,2,FALSE),""), IFERROR(VLOOKUP(T2,mappings!$A:$B,2,FALSE),""))` |

   (Columns A, B, C, K, N, O and the helper columns Q–V are typed by hand — no formula.)

   **Note on `Vendor`:** it holds the **media format** (VHS / DVD / Blu-Ray / 4K), not a company name. That's how the whole existing catalogue is set up, and it's what the website reads to show the format badge and to power the Format filter. Spelling matters — always pick from the dropdown, never type it.
...
1. Select columns **A through P** only (Handle … Genre). Do NOT include the helper columns Q–V (Format, Genre 1/2/3, Type, Extra tags).
...
- **New format:** add it to the `Format` dropdown's list of items (Data → Data validation on col Q). Then tell your developer, so the website's format badge learns to recognise it.
