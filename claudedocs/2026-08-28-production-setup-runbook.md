...
2. Configure the **Membership** method only — item-based credits, **allowance = 3 movies at a time** (revised 2026-09-03), unlimited/same-day swaps. **No Calendar, no Subscription, no Resale** — confirmed out of scope entirely (see CLAUDE.md).
3. Create the membership plan product — **$160/yr, item allowance 3** (revised 2026-09-03). **Note (2026-09-10):** this plan already exists on production as "Little Movie Club -- 1 Year" (Active); this step is a verification, not a build. Set it to **Active** status before launch — this was the exact bug that made membership signup dead on dev.
