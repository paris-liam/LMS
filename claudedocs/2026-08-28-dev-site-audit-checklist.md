...
- [x] **"New arrivals this week" (homepage) always renders empty.** Root cause: no product anywhere in the store carries the `new-arrival` tag (the `Rental` tag and `Videos` category were already correctly applied to ~2,996 products). **Resolved on dev 2026-08-28** — manually tagged 5 sample titles so the rail shows real content; full setup + client workflow + production steps written up in `claudedocs/lms-admin-instructions.md`. **2026-09-09: tagging decision reversed** — both tagging and 7-day expiry are now fully automated via two Flow workflows, live on production. Follow-up still open: build the same two Flow workflows on the dev store (only production is confirmed).
