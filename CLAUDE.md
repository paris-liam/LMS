...
Both storefronts sit behind Shopify's password page. Use these with `--store-password`, or to authenticate a `curl` session against the storefront (`POST /password` with `form_type=storefront_password&password=<pw>` into a cookie jar) when you need to inspect rendered pages, facets, or filter counts.

| Store | Storefront password |
|-------|---------------------|
| `p0wkgv-wy.myshopify.com` (production, `littlemoviestore.com`) | `shaoff` |
| `lms-sandbox-lutsfahz.myshopify.com` (dev) | `Lms` |
