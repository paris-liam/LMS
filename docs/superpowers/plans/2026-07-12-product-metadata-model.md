...
- **Facets (Search & Discovery):** exactly Format, Genre, Decade, Label/Distributor (one axis). Availability is a separate Supercycle track, out of scope here.
...
Creates all ten `custom.*` product metafield definitions in one idempotent script (re-runnable; "already taken" is treated as OK). Definitions are pinned and storefront-readable so they show in one panel on the product page and are readable by the theme and Search & Discovery.
...
- Produces (consumed by Tasks 2, 3, 5) — product metafields in the `custom` namespace:
  ...
  - `custom.label` `single_line_text_field` (choices)
...
```
Expected output includes exactly these rows:
```
...
In Shopify admin → **Apps → Search & Discovery → Filters → Add filter**, add each of these (Source = Metafield):
- `custom.format` → label "Format"
- `custom.genres` → label "Genre"
- `custom.decade` → label "Decade"
- `custom.label` → label "Label / Distributor"
...
- **Spec coverage:** Layer 0 native fields (guide, Step 1 of Task 6) ✓; Layer 1 + Layer 2 metafields (Task 1) ✓; condition split (Task 1 `media_condition` description + Task 3 comment) ✓; Layer 3 tag curation + collections (Task 4) ✓; Layer 4 Supercycle boundary (guide "What NOT to touch" + constraints) ✓; facets (Task 5) ✓; client ease-of-use (Task 6) ✓; theme impact (Task 3) ✓; migration naming (Notes) ✓.
