...
> **⏸️ DEFERRED — do not build this plan yet (decision 2026-07-16).** This entire plan is the back-in-stock waitlist button, which is paused. Two blockers: (1) `supercycle.uncommitted_inventory` doesn't flip off on rental yet, so the button's OOS detection has no signal (open Supercycle support item); (2) the capture mechanism the button feeds does not exist as designed — Shopify Flow has no contact-form trigger, so the `{% form 'contact' %}` approach in Task 2 is invalid and must be rebuilt (Shopify Forms → metaobject → Flow, or app-proxy, or 3rd-party app — see runbook Section C's deferred banner + C2 finding). **To resume:** confirm blocker (1) is fixed, pick a capture mechanism, then revise Task 2's form to match it before building. The boolean read logic below (`== true`) is correct and reusable; the `{% form 'contact' %}` markup is not.
>
> *Historical caveat (pre-deferral):* Supercycle support confirmed `uncommitted_inventory` is the right availability signal and updates on fulfillment; observed staleness is a write-lag they're reconciling. See `[[supercycle-uncommitted-inventory-integer]]`.
